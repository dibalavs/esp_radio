...
void bus_init_i2c(void)
{
    ...
    conf.clk_flags = 0;
    err = i2c_param_config(I2C_NO, &conf);
    ESP_LOGI(TAG, "Init I2C bus. (sda:%d, scl:%d) = %s", conf.sda_io_num, conf.scl_io_num, esp_err_to_name(err));
    ESP_ERROR_CHECK(err);
...
void bus_init_i2s(void)
{
    ...
    const i2s_chan_config_t chan_cfg = I2S_CHANNEL_DEFAULT_CONFIG(I2S_NO, I2S_ROLE_MASTER);
