      ...
      <tbody>
        <template v-for="(item, index) in sorted" :key="item.no">
          <tr role="button" data-href="#">
            ...
            <td @click="onSwitchStation(item.no)">{{ item.frequency }}Mhz</td>
...
<script>
import FmRadioToolbar from './fmradio_toolbar.vue'
...
export default {
  ...
  methods: {
    getItems() {
      fetch(this.api_prefix + "fmradio_list")
        .then((response) => {
          if (!response.ok) {
    ...
    onSwitchStation(radio_no) {
      fetch(this.api_prefix + "fmradio_set", {
        method: "POST",
        headers: new Headers({
          'Content-Type': 'application/json'
        }),
        body: JSON.stringify({radio_no: radio_no})
      })
    },
    onSwapStations(curr, prev) {
      fetch(this.api_prefix + "fmradio_move", {
        method: "POST",
        headers: new Headers({
          'Content-Type': 'application/json'
        }),
